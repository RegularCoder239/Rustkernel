...
static IDTS: PerCpu<IDT> = PerCpu::new(IDT::EMPTY);
static INTERRUPT_CONNECTION_METHS: Mutex<[Vec<SignalMethod>; 0x100]> = Mutex::new(
	[const { Vec::new() }; 0x100]
);

extern "x86-interrupt" fn handle_error_interrupt() {
...
impl IDT {
	const EMPTY: IDT = IDT {
		descriptors: [InterruptDescriptor::EMPTY; 0x100]
	};
	pub fn init(&mut self) -> &mut IDT {
		for idx in 0..0x100 {
	...
	fn setup_error_handlers(&mut self) {
		for idx in 0..31 {
			self.connect_handler(idx, handle_error_interrupt);
			self.set_ist(idx, 0x1);
		}
	}
...
pub fn connect_signal(index: usize, meth: fn()) {
	let mut lock = INTERRUPT_CONNECTION_METHS.lock();
	lock[index].push_back(meth);
}

pub fn handle_interrupt(vector: u8) {
	if let Some(lock) = INTERRUPT_CONNECTION_METHS.try_lock() {
		let methods = &lock[vector as usize];
